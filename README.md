...
```sh
cargo build --package tumuchain-node --release
```
...
```sh
./target/release/tumuchain-node -h
```
...
```sh
./target/release/tumuchain-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/tumuchain-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/tumuchain-node -ldebug --dev
```
...
$ ./target/release/tumuchain-node --dev --base-path ./my-chain-state/
